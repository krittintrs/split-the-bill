...
**Phase:** 10 — #33 rounding absorber shipped as v0.7.0: `computeBill()` now resolves rounding in two tiers instead of one flat per-peer ceiling. Item tier: each ticked item's own leftover (0..tickerCount−1 satang, from ceiling the item once then flooring every ticker's share) goes to that item's designated absorbing ticker — default the first ticker, organizer-overridable per item via `roundingAbsorberPeerId` on `LineItemInput`, falling back to the first ticker if the stored id is stale. Bill tier (only once every item is ticked): floors each peer's staged subtotal→SC→VAT total, then hands the bill-wide remainder — from item-ceiling overshoot and/or SC/VAT compounding, can be negative — to one bill-level absorber (default the organizer's self-peer, via `BillInput.roundingAbsorberPeerId`), guarded so no peer's total ever goes negative. New `BillResult.itemLeftovers` / `billLeftover` fields expose exactly when a picker is needed; a shared `RoundingLeftoverBadge` component (leftover chip → expandable picker with a labeled shuffle option) surfaces both in the organizer's matrix/card item rows and the เช็คกับใบเสร็จ section — peers only ever see the resulting tied totals, no picker. See ADR-0011 (refines ADR-0001). Before it, Phase 9 shipped #20 calculation breakdown + #34 calculation bug together as v0.6.0: `computeBill()` decomposed its pipeline into three visible stages (subtotal → +service charge → +VAT) at both bill level and per peer, exposed as `BillResult` fields (`subtotalSatang`, `serviceChargeSatang`, `vatSatang`, `peerBreakdowns`); VAT is always the residual line so the three displayed numbers sum exactly to the total. Wired into four UI surfaces: the organizer's percent inputs get a permanent "%" suffix plus 5%/10% SC and 7% VAT quick-fill chips, the เช็คกับใบเสร็จ section shows a stacked line-by-line breakdown instead of one opaque figure, both matrix tables (`/bills/[id]` and `/b/[id]` desktop) gained SC/VAT footer rows plus a conditional ส่วนลด row, and the peer's own claimed row on mobile shows their personal subtotal → SC → VAT breakdown. #34 turned out not to be a code bug (SC/VAT fields were typed as 100% instead of 5%/7% with no unit hint) — this UX work is its fix, so it closes alongside #20. Before it, v0.5.0 (PR #32) shipped #24 organizer self-peer. #21 grilled 2026-08-31 (see Decision Log); plan at `docs/plans/PLAN-21-create-bill-rpc-toast.md`. → next: #21 (planned, ready for an agent), #11 dashboard, #12 my debts
...
| Med | #33 | Rounding absorber: two-tier rounding (per-item + bill-level) so the checksum ties the receipt exactly | — | dev agent | DONE ✅ v0.7.0 |
...
| 2026-08-31 | #33: rounding resolved in two tiers instead of one flat ceiling per peer (ADR-0011, refines ADR-0001). Item tier — each ticked item's exact chargeable amount is ceil'd once, then every ticker's share is floored and the item's own leftover (0..tickerCount−1 satang) goes to that item's designated absorbing ticker (default first ticker, organizer-overridable, falls back if stale); a 1-ticker item degenerates to the old per-item ceiling exactly. Bill tier (only when every item is ticked) — floors each peer's staged subtotal→SC→VAT total and hands the bill-wide remainder (can be negative, from item-ceiling overshoot and/or SC/VAT compounding) to one bill-level absorber (default the organizer's self-peer), guarded so no peer's total goes negative. `line_items`/`bills` each gained `rounding_absorber_peer_id`; `get_bill()` reissued to return both | Considered a single largest-remainder pass across all peers (rejected in ADR-0001 already — peers who ticked identical items could land on different totals for no visible reason) and one bill-level absorber for everything (rejected — conflates unrelated items' rounding onto one peer who may not have touched them, shown by a two-group counter-example). The canonical Katsu fixture is unaffected (every item there has exactly one ticker); only fractional multi-ticker splits change | User + dev agent |
